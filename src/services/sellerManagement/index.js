...
export const getSellerListApi = async (data) => {
  const endPoint = `/api/v1/admin/seller/list?${new URLSearchParams(data)}`;
...
export const getSellerdetailsApi = async (data) => {
  const endPoint = `/api/admin/v1/seller/detail?${new URLSearchParams(data)}`;
   return await axios.get(endPoint, setJwtToken());
};

export const getSongdetailsApi = async (data) => {
  const endPoint = `/api/v1/admin/seller/uploaded-songs?${new URLSearchParams(data)}`;
  return await axios.get(endPoint, setJwtToken());
};
...
export const deleteUserApi = async (data) => {
  ...
  const endPoint = `/api/v1/admin/seller/delete/${userId}`;
